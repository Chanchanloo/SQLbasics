...
SELECT matchid, player FROM goal JOIN eteam ON (teamid = id)
WHERE teamid = 'GER'

#Show id, stadium, team1, team2 for just game 1012
SELECT DISTINCT id, stadium, team1, team2 FROM game JOIN goal ON (matchid = id)
WHERE  game.id = 1012
...
SELECT teamname, COUNT(gtime) FROM goal JOIN eteam ON (teamid = id) 
GROUP BY teamname

#Show the stadium and the number of goals scored in each stadium.
SELECT stadium, COUNT(gtime) FROM game JOIN goal ON (matchid = id)
GROUP BY stadium
